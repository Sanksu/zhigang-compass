...
(() => {
  ...
  // 右侧三卡片
  const views = [
    ["四视图切换", "panorama · techStack\nlevel · positionCenter"],
    ["匹配可视化", "环形图 · 雷达图\n热力图 · 甘特图"],
    ["响应式适配", "深浅双主题\n平板/移动 2D 固定"],
  ];
  views.forEach((v, i) => {
...
(() => {
  ...
  s.addText("另有：岗位职能域 15 个（LLM 语义命名，覆盖公开岗 100%）· 技能分类权威覆盖 23.9%（用户可见视图 91.7%）· 匹配黄金对 v1/v2 684 组", { x: M, y: 6.15, w: CW, h: 0.6, fontSize: 12.5, fontFace: FONT_BODY, color: MUTED, margin: 0, valign: "top" });
  s.addText("注：数据截至 08-29 图谱治理快照，汇报前以 graph_versions 最新快照复核", { x: M, y: 6.85, w: CW, h: 0.3, fontSize: 12, fontFace: FONT_BODY, color: "9AA9BA", margin: 0 });
})();

...
| [leiden.py](../../backend/app/services/graph_algorithms/leiden.py) | **阶段二 Leiden 条件替换**：同签名 `leiden(graph, resolution=1.0)`（igraph 1.0 + leidenalg 0.12，seed=0 确定性）。首轮验收未达标（旧数据 Q 落后 0.037）保持 louvain；**数据扩充后复验达标（08-12 晚）已切换 `algorithm=leiden`**（γ=0.992/mw=1.367，Q 0.2589 vs Louvain 0.0972、同质性 0.5536 vs 0.3812）；API 依赖缺失自动回退 louvain 一行回滚 | ✅ 完整（已启用） |

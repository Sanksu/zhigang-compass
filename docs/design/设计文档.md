...
| 维度 | 方法 | 默认权重 | 说明 |
|------|------|---------|------|
| 必备技能 | `must_score = Σ(w×sim) / Σ(w)`，w = log(source_count+1)；岗位无必备技能 → null（不判分） | 0.6 | 同义词集带权相似度映射，`Σ(w×sim)/Σ(w)` 已天然惩罚缺失；跨源多的核心技能权重更高（方案 B），source_count 默认 1 时等权退化 |
| 加分技能 | `nice_score = Top-K 覆盖率 Σ(w×sim) / Σ(w)`，K=10（跨源数降序取前 K） | 0.2 | Embedding 余弦 ≥ sim_threshold 视为匹配；Top-K 截断抗长尾稀释（08-22） |

...
| GET | `/position/{id}/skills` | 公开 | 岗位技能列表 | `necessity`(可选) | `[{skill_id, name, necessity, weight}]` |
| GET | `/skill/{id}` | 公开 | 技能节点详情 | — | `id`, `name`, `category`, `description` |
...
| GET | `/panorama` | 公开 | 全景视图（Neo4j 查询 + 300s Redis TTL 缓存，管理端写路径即时失效，见 10.3/11.3.5） | `limit`(默认100), `min_weight`(默认0.3) | `{nodes[], edges[], stats}` |
| GET | `/panorama?focus={position_id}` | 公开 | 以某岗位为中心的展开视图 | — | `{nodes[], edges[], stats}` |
| GET | `/view/{view_type}` | 公开 | 视图切换（后端过滤） | `view_type`: panorama/techStack/level/positionCenter | `{nodes[], edges[]}` |

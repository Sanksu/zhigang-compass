...
| 维度 | 方法 | 默认权重 | 说明 |
|------|------|---------|------|
| 必备技能 | `must_score = Σ(w×sim) / Σ(w)`，w = log(source_count+1) | 0.6 | 同义词集带权相似度映射，`Σ(w×sim)/Σ(w)` 已天然惩罚缺失；跨源多的核心技能权重更高（方案 B），source_count 默认 1 时等权退化 |
...
**必备技能匹配**（`must_score = Σ(w×sim) / Σ(w)`，w = log(source_count+1)）：同义词匹配采用带权相似度映射（别名级 1.0，语义级 0.85-1.0）。技能级权重按岗位来源源数平滑（方案 B），跨源多的核心必备技能命中/缺失影响更重；`source_count` 缺失或默认 1 时全技能等权（log2），退化为优化前行为（黄金集零回归）。`Σ(w×sim)/Σ(w)` 已天然惩罚缺失，无需额外比例惩罚。

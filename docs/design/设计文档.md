...
Z-score（标准分）为主要判定信号，`z = (f(t) - μ) / σ`，其中 f(t) 为当前 30 天窗口频次，μ/σ 为历史窗口均值/标准差。判定阈值：`z > 2.0 → emerging`，`z > 1.5 → rising`，`z < -1.5 → declining`。环比（MoM）辅助观察短期方向。频次 < 10 的技能受小基数保护不参与严格门控（严格门控要求 `jd_freq_ma3 ≥ 10`）；快照窗口不足无法计算 Z-score 时改用 Wilson score 冷启动兜底（见 §7.2.3/§7.2.4）。
...
#    有门槛：必备技能全缺失；无门槛：加分技能全未命中（A3：防无关候选占位）
if (must_total > 0 and must_score == 0) or (must_total == 0 and nice_score == 0):
    total = 0.0
    return total

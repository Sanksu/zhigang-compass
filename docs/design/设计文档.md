...
| `stable` | 稳定态 | jd_count ≥ 5 + source_diversity ≥ 2 + 连续 2 窗口频次波动 < 25% + skill_novelty < 0.2 | → `declining` | 是 |
...
**稳定化/衰退**：
  emerging → stable: 连续 2 窗口波动 < 25% AND source_diversity ≥ 2（+ jd_count ≥ 5 + skill_novelty < 0.2，见 §7.2.1 表格）
...
| emerging → stable | jd_count ≥ 5 + 连续 2 窗口波动 < 25% + source_diversity ≥ 2 + skill_novelty < 0.2（08-15 全量对齐，不再用 confidence ≥ 0.8——小基数岗位其他维度满分也能过 0.8，提前稳定） | 自动迁移 |

...
flowchart LR
    subgraph Input ["多源数据输入"]
        A1["招聘平台 JD<br/>国内 BOSS/智联/拉勾/脉脉 + 国际 LinkedIn/Indeed/Monster/Glassdoor"]
        A2["GitHub / SO"]
        A3["arXiv 论文"]
        A4["职业标准库 O*NET"]
...
5. **LLM 多 provider 同步重试链**：放弃本地 Ollama fallback（CPU 推理 30-60s 与超时阈值冲突 + GPU 硬件不确定），改用主讯飞 + 备 DeepSeek + 三 Qwen 三 provider 同步重试链。同步路由超时即报错（10s 上限），异步任务按优先级依次尝试（90s 上限）
...
**数据源矩阵**（3 国内 + 4 国际 + 3 社区 + 1 论文 + 3 课程 + O*NET，共 14 源，国内直连 + 国际走网络代理）：
...
| **B 级**（P1 补充） | 拉勾 | Scrapy + Playwright（公开搜索页） | 每日 | 同上 | 国内直连 | 交叉验证（国内，失败当日停止） |
...
>
> **网络代理方案（D1b：Rule 模式）**：开发环境配置 网络代理/V2Ray 系统代理（Rule 模式），国内平台（BOSS/智联/拉勾/脉脉/中国大学MOOC）走 DIRECT 直连，国际平台（LinkedIn/Indeed/Monster/Glassdoor/GitHub/SO/arXiv/O*NET/Coursera/edX）走 Proxy 出口。爬虫代码不感知代理，由 网络代理 在系统层完成分流。网络代理 失效时仅国际平台降级，国内平台不受影响。
>
> **平台 A/B/C 分级管理**：
> - **A 级（P0 主力，稳定源）**：BOSS、智联、Monster —— 3 平台保底 P0 单日 ≥ 60 条
> - **B 级（P1 补充，次稳定）**：拉勾、Indeed、Glassdoor —— 能跑就跑，失败当日停止
...
爬虫开发交付节奏（单人多源约束）：M2 已完成 7 招聘源贯通（BOSS/智联/Monster/Indeed/Glassdoor/脉脉/LinkedIn 公开页）+ 清洗管线 + 6 非招聘源（arxiv/github/stackoverflow/icourse163/coursera/edx）；M3 补齐拉勾 + 时滞/通胀调优 + LLM 抽取管线；M4 课程平台对接学习路径 + 观察池接入。
...
主备三个 provider 均通过 OpenAI 兼容 API 接入，配置存放于 `configs/llm_providers.yaml`：

| 优先级 | Provider | 模型 | 适用场景 | 备注 |
|--------|----------|------|---------|------|
| 主 | 讯飞星火 | v4.0 | 全场景（抽取/匹配/诊断） | 支持 function calling |
| 备 | DeepSeek | deepseek-chat | 全场景 | 支持 function calling |
...
| `source_diversity` | JD 独立源计数（BOSS/智联/拉勾等） | JD 跨源验证强度 |

...
- **完整审查批次 A（交付文档与新鲜环境降级修复）**：五路只读审查（后端安全/ETL 管线/前端/部署交付/测试 CI）后首轮修复——① **交付文档 neo4j 命令同步**：DEPLOY.md §6.0 手工恢复与 snapshots/README.md 中的 `--database=neo4j` flag 形式（已证触发 exit 64）统一改为位置参数 `--overwrite-destination=true neo4j`；② **restore_snapshot.sh 新鲜环境引导补齐**：新增 `llm_providers.yaml`/`runtime_settings.json` 缺失时从 `.example` 模板就位（防 compose 单文件挂载点目录化）+ `backend/models/sbert` 缺失时显著警告（语义匹配降级纯规则，快照导入与健康检查不受影响），成功提示由 `LLM_API_KEY` 改为指引 llm_providers.yaml；③ **DEPLOY.md 卫生项**：healthcheck start-period 描述 10s→90s、§6.0 dump 体积口径统一 129MB/44MB；④ **package_release.sh 重复块去重**。交付包内 DEPLOY.md/数据快照 README/restore_snapshot.sh 三副本已同步。

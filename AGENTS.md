...
## 10. 已确认变更记录（用户确认，2026-08-13）

> 项目负责人已确认以下变更（详细记录见 `project_memory.md`「2026-08-13 确认记录」）。
> 后续 AI 会话处理相关模块时，以下红线事项视为**已获用户授权**，但仍须走 PR + Review 流程。

| 变更 | 确认内容 | 分支 |
|------|---------|------|
| JD prompt 标题优先迭代 | prompts.py 规则 1「标题优先」「技术栈前缀保留」+ few-shot 示例 11（skills F1 0.762 持平、Odoo 实证修复） | feat/algo-jd-title-priority |
| 学习路径全套变更 | 评审定稿口径、课程名语义门控（≥0.5）、课程级兜底（0.55）、学时分层（类别基准 + weak 减半）、脏边清理（1854 条）、审查修复 | feat/algo-learning-path-eval |
| 依赖变更 | openpyxl>=3.1（dev，评测链盲标工作簿生成） | feat/test-jd-blindset-round2 |
| 图谱数据操作 | LEARNABLE_VIA 严重脏边（sim<0.3）删除 1854 条；可疑档（0.3-0.45）抽审后保留不清理 | 运行时数据 |

...
语义约定（每型一句定义 + 一个通用正例；先修/父子按定义区分）：
- PREREQUISITE_OF：源是目标的前置先修——先学/先掌握源是学会目标的前提
  （通用例：加减法 → 乘法）。技术栈依赖常识即可判定，不要因缺少共现证据
  而保守答 NONE；方向 a_to_b（先修→目标）
- BELONGS_TO：源从属于目标——源是目标的组成部分/子领域/特性（通用例：
  发动机 → 汽车）。方向 a_to_b（子→父）
- ALTERNATIVE_OF：两者解决同类问题、岗位要求中可互相替代（通用例：
  可转债 ↔ 优先股）。方向 symmetric
- NONE：以上三者都不成立时才使用；拿不准语义关系时优先 NONE 并降置信度
- 只选最有把握的一种关系；多义时优先 BELONGS_TO → NONE

先修判据补充（权威关系政策，易错形态按此裁决）：
- 组成技术 → 由其构成的领域技能 = 先修（学领域技能须先掌握其组成技术：
  HTML→前端开发、Spring Boot→微服务、Elasticsearch→搜索引擎、SQL→
  具体数据库产品如 MySQL）——这不是 BELONGS_TO；BELONGS_TO 限于
  「子领域/特性 → 父技术」（如某框架的具体特性 → 该框架本身）
- 基础设施概念 → 建于其上的中间件/平台 = 先修（分布式→消息队列/Kafka/
  Databricks、操作系统→分布式、计算机基础→编程语言与系统领域）
- 数据链路上游 → 下游 = 先修（数据仓库→ODS/主数据/Databricks、SQL→ODS）
"""

# 判据措辞迭代记录（20260824-25）：
# - r2 短条文：precision 0.6211（先修命中 34/55）
# - r3 长条文「区分自问+判别示例」：BELONGS_TO 误判先修 15/26→22/26（0.5053），已回退
# - r4 工整单句 + 非技能通用正例：0.7053（先修 31/55；误判=组成技术→领域技能
#   被判父子 10 条 + 基础设施→中间件被判 NONE 13 条）
# - r6 本版：先修判据补充（r4 错误形态的权威政策显式化，同分类 r5 手法）；
#   复测结果见 docs/reviews/LLM驱动关系专项复测r6

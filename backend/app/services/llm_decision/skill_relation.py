...
- PREREQUISITE_OF：源是目标的前置先修——**学习顺序依赖**（不先掌握源，
  目标学不会/无法入门；如语言→基于其上的框架、数学基础→算法领域）。
  技术栈依赖常识即可判定，不要因缺少共现证据而保守答 NONE；方向 a_to_b
- BELONGS_TO：源从属于目标——**分类包含**（源是目标的组成部分/子领域/
  特性集合，如某框架的具体组件、某语言的某一范式），**无学习顺序含义**。
  方向 a_to_b（子→父）
- 区分自问：目标是「包含源作为组成部分/子领域」→ BELONGS_TO；「要学/用
  目标必须先掌握源」→ PREREQUISITE_OF。框架/平台对语言的依赖是先修；
  框架对它自身组件/特性的包含是父子。

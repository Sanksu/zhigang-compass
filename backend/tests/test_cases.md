...
> 模块缩写：DC（数据采集）/ KG（知识图谱）/ PD（新岗位发现）/ EV（演化）/ RM（简历匹配）/ HL（幻觉防控）/ SI（系统集成）/ GRILL（治理与一致性加固，grilling 决策补充）
> 详细执行计划见 [执行计划.md](../../docs/design/执行计划.md)。
...
| 治理与一致性加固 | GRILL | TC-GRILL-01 ~ TC-GRILL-06 | 缓存一致性/T+1 窗口/LLM 降级/审计（2026-08-29 grilling 决策补充，见文末「新增用例」节） |

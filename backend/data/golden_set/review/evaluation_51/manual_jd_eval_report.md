...
```json
{
  ...
  "skills_micro": {
    ...
    "f1": 0.9412587412587412
  },
  "skills_average_sample_f1": 0.936538186433163,
  "bonus_skills_micro": {
    "tp": 99,
    "fp": 59,
    "fn": 31,
  ...
  "bonus_skills_average_sample_f1": 0.6781532253941652,
...
- jd_030: title raw=True, normalized=True; skills TP/FP/FN=['AIGC', 'Prompt', '数据分析', '数据标注', '视频生成', '音频生成']/['竞品评测']/[], F1=0.9231; bonus TP/FP/FN=['Python', 'SQL', '数据可视化', '自动化评测']/[]/['多模态模型', '数据标注'], F1=0.8000; education=True
...
- public_003: title raw=True, normalized=True; skills TP/FP/FN=['Linux', '图计算', '数据挖掘', '机器学习', '深度学习', '自然语言处理']/['Hive', 'PyTorch', 'TensorFlow']/[], F1=0.8000; bonus TP/FP/FN=['Hive', 'PyTorch', 'TensorFlow', '大语言模型']/['互联网风控', '图像', '平台治理', '推荐', '搜索引擎', '智能客服', '用户增长', '计算广告']/['广告算法', '推荐算法', '计算机视觉', '风控算法'], F1=0.4000; education=True
- public_004: title raw=True, normalized=True; skills TP/FP/FN=['Ansible', 'ELK', 'Git', 'Grafana', 'Jenkins', 'Kubernetes', 'Linux', 'Prometheus', 'Python', 'Shell', '可观测性', '容器编排']/['容器化']/[], F1=0.9600; bonus TP/FP/FN=['AWS', 'Azure', 'GCP', '微服务']/[]/[], F1=1.0000; education=True
...
- public_006: title raw=True, normalized=True; skills TP/FP/FN=['数据分析', '机器学习', '统计学', '项目管理']/[]/['数据可视化', '数据建模'], F1=0.8000; bonus TP/FP/FN=[]/['CDA', 'CPDA', '项目管理']/[], F1=0.0000; education=True
- public_007: title raw=True, normalized=True; skills TP/FP/FN=['A/B测试', 'Apache Spark', 'CNN', 'Elasticsearch', 'GBDT', 'LR', 'Pandas', 'PyTorch', 'Python', 'RNN', 'SQL', 'TensorFlow', 'Transformer', '多模态模型', '大语言模型', '机器学习', '模型', '模型融合', '模型评估', '深度学习', '特征工程']/[]/[], F1=1.0000; bonus TP/FP/FN=[]/['多模态模型', '大语言模型']/['国产算力研发'], F1=0.0000; education=True
...
- r2_003: title raw=False, normalized=True; skills TP/FP/FN=['AGENT', 'AWQ', 'Apache Kafka', 'Chroma', 'DeepSeek', 'Docker', 'Elasticsearch', 'Embedding', 'FastAPI', 'Function Calling', 'GLM', 'GPTQ', 'Hybrid Search', 'INT4', 'INT8', 'Kubernetes', 'LangChain', 'LlamaIndex', 'Milvus', 'Multi-Agent', 'PyTorch', 'Python', 'Qwen', 'RESTful API', 'React', 'Redis', 'Reflexion', 'SSE', 'TGI', 'Tool Calling', 'Transformer', 'WebSocket', 'vLLM', '向量数据库', '大语言模型', '异步编程', '推理加速', '检索增强生成', '模型量化']/['KV Cache', '寒武纪', '昇腾', '高并发']/[], F1=0.9512; bonus TP/FP/FN=['CUDA', 'MindIE', 'OCR', 'Triton', 'VLM', '昇腾CANN']/['Layout Analysis']/[], F1=0.9231; education=True
- r2_004: title raw=True, normalized=True; skills TP/FP/FN=['CTS', 'Cadence', 'ICC2', 'Innovus', 'P&R', 'Perl', 'STA', 'Tcl', '布局布线', '平面规划', '数字后端', '时序分析', '时序收敛', '物理', '物理验证', '电源分析']/[]/[], F1=1.0000; bonus TP/FP/FN=[]/['Cadence']/['DDR', 'PCIE', 'PHY'], F1=0.0000; education=True
- r2_005: title raw=True, normalized=True; skills TP/FP/FN=['ByteTrack', 'C++', 'CNN', 'DeepSORT', 'NCNN', 'ONNX', 'OpenCV', 'PyTorch', 'Python', 'TensorRT', 'Transformer', 'YOLO', '剪枝', '去雾', '图像分割', '图像增强', '图像识别', '多目标跟踪', '模型量化', '特征提取', '目标检测', '目标跟踪', '防抖', '障碍物识别']/[]/[], F1=1.0000; bonus TP/FP/FN=['SLAM', '视觉定位', '语义分割']/['C++']/[], F1=0.8571; education=True
...
- r2_007: title raw=True, normalized=True; skills TP/FP/FN=['BI报表', 'Excel', 'NumPy', 'PPT', 'SQL', '指标体系', '指标体系搭建', '数据仓库', '数据仓库建模', '数据分析', '数据建模', '数据清洗', '数据透视', '统计学']/[]/['数仓建模', '数据可视化'], F1=0.9333; bonus TP/FP/FN=['NumPy', 'Pandas', 'Python']/['BI报表']/['BI', '指标体系', '数据挖掘'], F1=0.6000; education=True
- r2_008: title raw=True, normalized=True; skills TP/FP/FN=['AGENT', 'Agent开发', 'DeepSeek', 'Docker', 'FastAPI', 'LangChain', 'Llama', 'LlamaIndex', 'LoRA', 'Milvus', 'MySQL', 'OCR', 'OpenAI', 'PyTorch', 'Python', 'QLoRA', 'Qwen', 'Redis', 'TensorFlow', 'pgvector', '向量检索', '大语言模型', '提示工程', '数据治理', '文心一言', '文档切片', '检索增强生成', '自然语言处理', '通义千问', '重排序']/[]/['OpenAI API'], F1=0.9836; bonus TP/FP/FN=['SFT微调', '模型量化']/['显存推理']/['医药行业大模型', '显存'], F1=0.5714; education=True
...
- r2_011: title raw=True, normalized=True; skills TP/FP/FN=['C++', 'CAN', 'Jetson', 'Linux', 'MAVROS', 'Navigation2', 'OpenCV', 'PCL', 'PX4', 'Python', 'ROS', 'SLAM', 'SPI', 'UART', '传感器融合', '卡尔曼滤波', '激光雷达', '路径规划', '运动控制']/['CI/CD', 'CMake', 'Catkin', 'DDS', 'Gazebo', 'Git', 'Isaac Sim', 'RViz', 'gtest', 'rostest', 'rqt']/[], F1=0.7755; bonus TP/FP/FN=[]/[]/[], F1=1.0000; education=True
...
- r2_013: title raw=False, normalized=True; skills TP/FP/FN=['C++', 'CNN', 'LiDAR', 'ORB', 'OpenCV', 'OpenMVS', 'PyTorch', 'Python', 'SFM', 'SIFT', 'SURF', 'Transformer', '三维重建', '传感器融合', '光束平差', '多模态数据配准', '多视几何', '密集匹配', '影像解译', '摄影测量', '机器视觉', '深度学习', '特征提取', '畸变校正']/[]/['GPU加速', '相机标定'], F1=0.9600; bonus TP/FP/FN=[]/[]/[], F1=1.0000; education=True
...
- r2_018: title raw=True, normalized=True; skills TP/FP/FN=['Git', 'MySQL', 'PostgreSQL', 'Python']/[]/['多线程', '并发编程', '并行计算', '异步编程', '高性能计算'], F1=0.6154; bonus TP/FP/FN=['AWS', 'Azure', 'Django', 'FastAPI', 'Flask', 'GCP', '单元测试']/['Agile', '契约测试', '集成测试']/[], F1=0.8235; education=True
...
- r2_020: title raw=False, normalized=True; skills TP/FP/FN=['C++', 'Linux', 'NPU推理', 'OCR', 'ONNX', 'OpenCV', 'Python', 'RKNN', '人体姿态估计', '人脸识别', '图像分割', '图像分类', '图像处理', '多模态模型', '少样本学习', '嵌入式部署', '度量学习', '时序建模', '目标检测', '自监督学习', '表情分析', '视频分类', '视频动作检测', '视频异常检测', '视频行为理解', '迁移学习', '重识别']/['机器视觉']/['模型量化', '目标跟踪'], F1=0.9474; bonus TP/FP/FN=['ISP', 'MNN', 'NCNN', 'OpenVINO', 'TensorRT', 'VLM', '大语言模型', '强化学习']/['LMM', 'RKNN', '图像质量评价', '机器人', '相机采集']/[], F1=0.7619; education=True
- r3_001: title raw=True, normalized=True; skills TP/FP/FN=['C', 'C++', 'Java', 'Python', '数据结构']/[]/[], F1=1.0000; bonus TP/FP/FN=[]/['蓝桥杯']/[], F1=0.0000; education=True
- r3_002: title raw=False, normalized=True; skills TP/FP/FN=['', 'AI模型', 'Python', '分子动力学模拟', '反应路径探索', '并行化', '微分方程求解', '数值方法', '数值积分', '数据收集', '机器学习', '机器学习势函数', '构象搜索', '模型搭建', '线性代数求解器', '自由能计算', '蒙特卡洛方法', '量子化学计算']/['优化']/[], F1=0.9730; bonus TP/FP/FN=[]/['多肽药物', '小分子药物']/[], F1=0.0000; education=True
- r3_003: title raw=False, normalized=True; skills TP/FP/FN=['GIS', 'GeoServer', 'GeoWebCache', 'Hadoop', 'IO', 'InfluxDB', 'JVM调优', 'Java', 'MongoDB', 'MyBatis', 'MySQL', 'MySQL Spatial', 'Oracle Spatial', 'PostGIS', 'PostgreSQL', 'RESTful API', 'Redis', 'Spring Boot', 'Spring Cloud', 'TDengine', '叠加分析', '多线程', '容器化', '数据格式解析', '查询性能', '消息', '空间查询', '空间索引', '系统架构调优', '缓冲区分析', '集合']/['BUFR', 'GRIB', 'HDF5', 'Micaps', 'NetCDF']/[], F1=0.9254; bonus TP/FP/FN=['BUFR', 'GRIB', 'GeoServer', 'GeoWebCache', 'HDF5', 'Micaps', 'NetCDF']/[]/[], F1=1.0000; education=True
...
- r3_006: title raw=True, normalized=True; skills TP/FP/FN=['Apache Kafka', 'CAP理论', 'Docker', 'Go', 'Java', 'Kubernetes', 'MySQL', 'PostgreSQL', 'Postman', 'Python', 'RESTful API', 'RabbitMQ', 'Redis', 'Spring Boot', 'Spring Cloud', 'Swagger', '分布式', '分布式系统', '微服务', '性能调优', '负载均衡']/[]/[], F1=1.0000; bonus TP/FP/FN=[]/['Go', 'Python', '文旅领域']/[], F1=0.0000; education=True
...
- r3_008: title raw=False, normalized=True; skills TP/FP/FN=['', 'AGENT', 'CAE', 'GNN', 'PINN', 'VLM', '代理模型', '向量检索', '多模态模型', '大语言模型', '工业数据治理', '工具调用', '推理', '数据安全', '数据治理', '有限元仿真', '检索增强生成', '模型工程化', '模型微调', '知识图谱', '系统集成']/['Agent编排', '优化']/['模型对齐'], F1=0.9333; bonus TP/FP/FN=[]/['CAE', '有限元仿真']/['CAE仿真', '有限元分析'], F1=0.0000; education=True
- r3_010: title raw=False, normalized=True; skills TP/FP/FN=['AI', 'BDD', 'CI/CD', 'Claude', 'Java', 'Microsoft Copilot', 'Python', 'Selenium']/[]/['Cucumber', 'Cursor', '接口自动化测试'], F1=0.8421; bonus TP/FP/FN=[]/[]/[], F1=1.0000; education=True
- r3_011: title raw=False, normalized=False; skills TP/FP/FN=[]/[]/[], F1=1.0000; bonus TP/FP/FN=[]/[]/[], F1=1.0000; education=True
- r3_012: title raw=False, normalized=True; skills TP/FP/FN=['Airflow', 'CI/CD', 'Databricks', 'PySpark', 'Python', 'SQL', 'Spark SQL', 'dbt']/[]/[], F1=1.0000; bonus TP/FP/FN=[]/[]/['Azure'], F1=0.0000; education=True
...
- r3_005: skills F1=0.7500; FP=['销售流程']; FN=['市场分析']
...
- model-added skills not in human gold: 13

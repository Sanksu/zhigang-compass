...
| r2_014 React 细化 | 保留 React 细化 | React前端开发工程师（保持） |
...
## AI 复核口径评测基线（终审修正后，2026-08-13，`reports/eval_jd_llm_20260813_1842.json`，annotator=AI）

| 指标 | round2 20 条 | 修正前 |
|---|---|---|
| skills F1 | **0.7577**（P 0.7070 / R 0.8162） | 0.7466 |
| bonus F1 | 0.7273 | 0.6832 |
| title_norm | 0.5500 | 0.5000 |
| education | 0.9500 | 0.9500 |

> title 失配 9/20 明细（prompt 迭代素材）：数字后端→物理设计工程师、图像算法→图像识别算法、Java开发→全栈开发、机器人算法→机器人控制算法、SLAM算法→机器人系统工程师、摄影测量算法→机器视觉算法、React前端→前端开发、AI全栈→客户端开发、Python→计算金融软件——抽取器倾向按正文职责推导/泛化，未优先采用招聘标题语义；React/SLAM 等细化词丢失。round2 为更难的真实多样样本（含英文/双语 JD、无 REQ 段），数字供终审与 prompt 迭代参考，**非人工终审结果**。终审后重跑出合并口径（12+20=32 条）F1。

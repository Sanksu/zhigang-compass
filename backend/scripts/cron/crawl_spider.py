...
调用方式：
    ...
    # 关键词轮询（08-19 起 LinkedIn 使用，替代空关键词全量抓取+过滤）：
    uv run python scripts/cron/crawl_spider.py linkedin_public 50 python,java,golang
...
async def enqueue_crawl(
    ...
    try:
        ...
        if keywords:
            # 逗号分隔 → crawl_platform 转 -a keywords=a,b,c → base_spider 拆分
            kwargs["keywords"] = [k.strip() for k in keywords.split(",") if k.strip()]
        job = await client.enqueue_job("crawl_platform", **kwargs)
        logger.info(f"[crawl_spider] 已入队 crawl_platform spider={spider_name} "
                    f"keywords={kwargs.get('keywords')} job_id={job.job_id}")
    finally:
        await client.close()
...
def main() -> int:
    if len(sys.argv) < 2:
        logger.error("用法: python crawl_spider.py <spider_name> [max_results] [keywords]")
    ...
    try:
        asyncio.run(enqueue_crawl(spider_name, max_results, keywords))
        return 0

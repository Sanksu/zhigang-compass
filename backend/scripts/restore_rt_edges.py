...
logger = setup_logging("restore_rt_edges")
...
def main() -> None:
    ...
    with neo4j_driver.session() as session:
        ...
        if total == 0:
            logger.info("无 :rt 边，无需恢复。")
            return

        logger.info(f"发现 :rt 边 {total} 条，按端点标签对映射：")
        unknown = []
        for c in combos:
            rel = next((r for (s, d), r in _RESTORE_RULES if s == c["src"] and d == c["dst"]), None)
            if rel is None:
                unknown.append(c)
                logger.warning(f"  {c['src']} -[:rt]-> {c['dst']}: {c['n']} 条 → 未配置恢复规则，跳过（保留）")
            else:
                logger.info(f"  {c['src']} -[:rt]-> {c['dst']}: {c['n']} 条 → {rel}")
        ...
        for c in combos:
            ...
            logger.info(f"  已恢复 {c['src']} → {c['dst']}: {fixed} 条")

        # ---- 验证 ----
        rt_left = _scan_rt(session)
        left = sum(c["n"] for c in rt_left)
        logger.info("\n验证：")
        logger.info(f"  :rt 残留: {left}（应 0）")

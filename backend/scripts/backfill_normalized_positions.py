...
- --apply：单事务内对清单目标行执行条件 UPDATE（jsonb_set 仅加键），WHERE
  重查 extraction 存在 / normalized_position 空 / skipped 非真 / position_name
  非空；不覆盖非空旧值。实际更新行数 ≠ --expect-updated 时整体回滚。
- --verify：只读校验（目标值一致 / 排除行未变 / 原始岗位名未变 /
  非目标快照哈希未变）。
- --rollback：按清单整快照恢复，仅当现值 == 本次写入值。
...
def _classify(snapshot: dict, value: str) -> tuple[str, str]:
    """返回 (classification, old_normalized)。"""
    old = str(snapshot.get("normalized_position") or "").strip()
    if not value:
        extraction = snapshot.get("extraction") or {}
        if extraction.get("skipped"):
            return "empty_skipped", old
        if not str(extraction.get("position_name") or "").strip():
            return "empty_raw_name", old
        return "rejected_by_rules", old
    if old and old == value:
        return "existing_equal", old
    return "target", old
    ...
    while True:
        async with async_session_factory() as session:
            ...
            for row in rows:
                ...
                value = normalized_position_from_snapshot({**snap, "normalized_position": ""})
                classification, old = _classify(snap, value)
                lines.append(
                    {
                        "jd_raw_id": row.id,
                        "source": row.source,
...
async def apply_manifest(path: Path, expect_updated: int) -> None:
    ...
    targets = [line for line in lines if line["classification"] == "target"]
    ...
    statement = text(
        ...
        WHERE id = :id
          ...
          AND COALESCE(NULLIF(btrim(snapshot->>'normalized_position'), ''), '') = ''
    ...
    async with engine.begin() as conn:
        ...
        for line in targets:
            result = await conn.execute(
                statement, {"id": line["jd_raw_id"], "value": line["new_normalized"]}
            )
...
async def verify_manifest(path: Path) -> None:
    ...
    lines = _load_manifest(path)
    targets = [line for line in lines if line["classification"] == "target"]
    others = [line for line in lines if line["classification"] != "target"]
    failures: list[str] = []
    ...
    async with async_session_factory() as session:
        if targets:
            ...
            for line in targets:
                ...
                restored = {k: v for k, v in snap.items() if k != "normalized_position"}
                if _snapshot_hash(restored) != line["snapshot_hash"]:
                    failures.append(f"{line['jd_raw_id']}: 非 normalized 部分快照哈希不一致")
        ...
        if others:
            ...
            for line in others:
                ...
                if snap is None:
                    failures.append(f"{line['jd_raw_id']}: 行不存在")
                    continue
                if line["classification"] == "existing_equal":
                    # 计划时已存在且与规则一致的值：校验其仍等于清单值（不要求空白）
                    if str(snap.get("normalized_position") or "") != line["new_normalized"]:
...
async def rollback_manifest(path: Path) -> None:
    ...
    statement = text(
        ...
        WHERE id = :id AND snapshot->>'normalized_position' = :new_value
        """
    )
    async with engine.begin() as conn:
        rolled = 0

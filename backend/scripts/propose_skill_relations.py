"""技能关系 LLM 提议脚本（PR6：JD 共现候选 → LLM 关系判定 → proposal 决策记录）。
...
def propose(limit: int = DEFAULT_LIMIT) -> dict:
    """执行一轮关系提议；返回摘要（like shadow worker 语义）。"""
    ...
    cooccurrence, known = fetch_relation_inputs(neo4j_driver)
    ...
    prerequisite_map = _relation_direction_map(neo4j_driver)

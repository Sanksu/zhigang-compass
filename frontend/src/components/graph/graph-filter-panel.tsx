  ...
  return (
    <section className="absolute left-3 top-3 z-20 w-[calc(100%-1.5rem)] max-w-64 overflow-hidden rounded-lg border border-border/80 bg-canvas/90 shadow-md backdrop-blur-xl sm:left-4 sm:top-4 sm:w-56" aria-label="图层探索器">
      ...
      >
        ...
        {activeFilterCount > 0 && (
          <span className="rounded-full bg-ink px-1.5 py-0.5 font-mono text-[10px] text-canvas" aria-label={`${activeFilterCount} 个过滤条件已启用`}>
            {activeFilterCount}
          </span>
        )}
        {typeof visibleCount === 'number' && <span className="font-mono text-[11px] text-ink-faint">{visibleCount} 可见</span>}
        ...
        <div id="graph-layer-controls" className="max-h-[min(70vh,480px)] space-y-4 overflow-y-auto border-t border-border/70 px-3 py-3">
          <div>
            <div className="mb-2 flex items-center justify-between">
              <label htmlFor="graph-min-weight" className="text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-muted">关系强度</label>
              <span className="font-mono text-[11px] text-ink-secondary">≥ {minWeight}</span>
            </div>
            <input
              id="graph-min-weight"
              type="range"
          ...
          {onToggleStatus && (
            <fieldset>
              <legend className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-muted">岗位状态</legend>
              <div className="grid grid-cols-2 gap-1">
                {ALL_STATUSES.map((status) => {
                  const visible = !hiddenStatuses.has(status.value)
                  return (
                    <label key={status.value} className={cn('flex cursor-pointer items-center gap-2 rounded-md border px-2 py-1.5 text-[12px] transition-colors', visible ? 'border-border bg-canvas text-ink-secondary' : 'border-transparent bg-subtle text-ink-faint line-through')}>
                      <input
                        type="checkbox"
                        checked={visible}
          ...
          <fieldset>
            <legend className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-ink-muted">技能与关系</legend>
            <div className="space-y-1">
              {onToggleSoftSkills && (
                <label className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-[12px] text-ink-secondary hover:bg-subtle">
          ...
          <div className="flex items-center justify-between border-t border-border/70 pt-2">
            ...
            <button type="button" onClick={onReset} disabled={activeFilterCount === 0} className="flex items-center gap-1 rounded px-1.5 py-1 text-[11px] font-medium text-ink-muted hover:bg-subtle hover:text-ink disabled:cursor-default disabled:opacity-35">
